...
btnSearch.addEventListener('click', (e) => {
  // Один параметр в объявлении стрелочной функции можно не оборачивать в круглые скобки и записать без них
...
// Точка с запятой традиционно проставляется в конце операций с присваиванием, вызовах функций и методов и т.д. – почти во всех строках, кроме завершения конструкций условий, циклов и объявлений функций
...
function getDetailsFilm() {
  ...
  // Кнопке с деталями стоит установить курсор указательный палец, чтобы пользователям было понятно, что по ней можно кликнуть, а также стоит добавить некоторые стили по наведению курсора, чтобы был некоторый интерактив

  // Отделяйте конструкци условий и циклы от остального кода на том же уровне вложенности пустыми строками, так ваш код станет более наглядным и лёгким в работе
  for (let btn of details) {
    ...
    // Одинаковые обработчики на большом кол-ве одинаковых элементов – это прямой случай для делегирования событий – https://learn.javascript.ru/event-delegation
  }

  // Когда пользователю захочется узнать намного больше о фильме и он захочет скопировать имя актёра из данного детального описания, у него к сожалению мало что получится, т.к. данное окно закрывается по любому клику. Поле для клика нужно было поместить за окно с детальной информацией о фильме.
}
...
function ajaxDetails(url) {
  ...
  request.onload = function () {
    ...
    if (request.status == 200) {
      ...
      // убираю скролл у body, если открыто модальное окно
      if (modalContainer) {
